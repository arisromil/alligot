...
@Table(name = "PHOTOS")
@NamedQueries({
		@NamedQuery(name = "info.galleria.domain.Photo.findAllPhotos", query = "SELECT p FROM Photo p"),
		@NamedQuery(name = "info.galleria.domain.Photo.findAllPhotosByAlbum", query = "SELECT p FROM Photo p JOIN p.album a WHERE a.albumId = :albumId") })
