...
@Table(name = "ALBUMS")
@NamedQueries({
		@NamedQuery(name = "info.galleria.domain.Album.findAllAlbums", query = "SELECT a FROM Album a"),
		@NamedQuery(name = "info.galleria.domain.Album.findAllAlbumsByOwner", query = "SELECT a FROM Album a JOIN a.user u WHERE u.userId = :userId") })
